...
description: Use this agent when you need expert guidance on LangGraph or LangChain implementation details, API usage, patterns, or troubleshooting. This agent is specifically designed for questions about:\n\n- LangGraph state management, checkpointing, and memory patterns\n- Multi-agent orchestration and subgraph composition\n- Human-in-the-loop (HITL) flows with interrupt/resume semantics\n- LangChain 1.0-alpha (@next) package usage and migration\n- Tool integration and custom node development\n- Graph compilation, streaming, and execution patterns\n- Type safety with Zod schemas and TypeScript\n- Debugging graph execution and state transitions\n\nExamples of when to use this agent:\n\n<example>\nContext: User is implementing a new subgraph and needs to understand state management patterns.\nuser: "I'm creating a new subgraph for document processing. Should I create a separate state file like planner/state.ts or use the parent state?"\nassistant: "Let me consult the langgraph-expert agent to provide guidance on subgraph state management patterns."\n<commentary>\nThe user is asking about LangGraph architecture patterns. Use the langgraph-expert agent to provide authoritative guidance based on the project's internal documentation and LangChain source code.\n</commentary>\n</example>\n\n<example>\nContext: User encounters an error with checkpoint persistence.\nuser: "I'm getting a 'checkpoint not found' error when trying to resume a thread. Here's my code: [code snippet]"\nassistant: "I'll use the langgraph-expert agent to diagnose this checkpointing issue."\n<commentary>\nThis is a LangGraph-specific technical issue. The langgraph-expert should analyze the code against documented patterns in documentation/langgraph/ and provide a solution.\n</commentary>\n</example>\n\n<example>\nContext: User wants to implement a new HITL pattern.\nuser: "How do I implement a multi-step approval flow where users can approve, reject, or request modifications?"\nassistant: "Let me engage the langgraph-expert agent to explain HITL patterns and provide implementation guidance."\n<commentary>\nThis requires deep knowledge of LangGraph's interrupt/resume semantics. The langgraph-expert should reference documentation/langgraph/07-human-in-the-loop.md and provide concrete examples.\n</commentary>\n</example>\n\n<example>\nContext: User is upgrading from legacy LangChain to 1.0-alpha.\nuser: "I have this old LangChain code using AgentExecutor. How do I migrate it to LangGraph 1.0-alpha?"\nassistant: "I'm going to use the langgraph-expert agent to provide migration guidance for LangChain 1.0-alpha."\n<commentary>\nThis requires expertise in both legacy and 1.0-alpha patterns. The langgraph-expert should reference the @next packages and internal documentation to provide accurate migration steps.\n</commentary>\n</example>
tools: Bash, Edit, Write, NotebookEdit, Glob, Grep, Read, WebFetch, TodoWrite, WebSearch, BashOutput, KillShell, mcp__ide__executeCode, mcp__ide__getDiagnostics, ListMcpResourcesTool, ReadMcpResourceTool
model: sonnet
color: orange
---
