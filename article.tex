...
\Authors{Berkay [Surname]\textsuperscript{1}}
\Keywords{LangGraph --- Deep Research --- Multi-Agent Systems (MAS)}
...
Large language models (LLMs) have demonstrated remarkable capabilities in summarisation and question answering, yet they often struggle with multi-step research tasks that require sustained planning, deep information retrieval, and synthesis across disparate sources. Single-shot generation strategies frequently succumb to hallucinations or superficiality when faced with complex queries. To address these limitations, we introduce \textit{SynGraph}, an agentic workflow built on LangGraph \cite{wu2024langgraph} that formalises the research process as a structured, stateful program.
...
\textbf{Opportunities.} Future work will focus on integrating the Model Context Protocol (MCP) to allow dynamic attachment of domain-specific tools (e.g., internal databases, code repositories). We also plan to implement a more sophisticated rate limiter backed by persistent storage to handle higher concurrency levels. Finally, exposing the "interrupt" events in a dedicated UI would allow for more granular human-in-the-loop control during the research process.
